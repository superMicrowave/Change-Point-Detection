...
for fold_num in range(1, 7):
    ...
    # get best L1 rate
    L2_rate = Selector2(model, PATH, criterion, train_data, train_label, 
                       epoch, L2_rate_list).__call__()
    print(L2_rate)
    ...
    optimizer = optim.Adam(model.parameters(),  lr= 1e-4, weight_decay = L2_rate)
    test_losses = []
    avg_test_losses = []
    ...
    with torch.no_grad():
        ...
        L2_regulizaton_acc.append(accuracy/num_test * 100)
        print(accuracy/num_test * 100)
    
# this fucntion output the csv file
L2_output = pd.DataFrame(test_output_list[0])
L2_output = OutputFile(L2_output, test_output_list)
L2_output.to_csv(argv + '/Outputs/L2_regModel.csv', index = None, header = False) 
